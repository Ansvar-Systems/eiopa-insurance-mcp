# Multi-stage build for eiopa-insurance-mcp
...
ENV EIOPA_DB_PATH=/app/data/eiopa.db

EXPOSE 8382

HEALTHCHECK --interval=30s --timeout=10s --start-period=10s --retries=3 \
  CMD node -e "fetch('http://localhost:8382/health').then(r=>r.ok?process.exit(0):process.exit(1)).catch(()=>process.exit(1))"
